...
namespace BS.Infra.Services.CRM
{
    public class ContactsService
    {
        ...
        {
            string sql = $@"SELECT C.*,EVT.VALUE_NAME CATEGORY_NAME, COUNT(CA.CONTACT_ID)HAS_ADDRESS
                    FROM CONTACTS C
					JOIN ENTITY_VALUE_TEXT EVT ON C.CONTACT_CATEGORY_ID = EVT.VALUE_ID
                    JOIN CONTACT_ADDRESS CA ON C.ID = CA.CONTACT_ID
                    ORDER BY C.CONTACT_CATEGORY_ID,C.CONTACT_GROUP,C.CONTACT_NAME";
            sql = @"SELECT C.*,EVT.VALUE_NAME CATEGORY_NAME, COUNT(CA.CONTACT_ID)HAS_ADDRESS
                    FROM CONTACTS C
					JOIN ENTITY_VALUE_TEXT EVT ON C.CONTACT_CATEGORY_ID = EVT.VALUE_ID
                    LEFT JOIN CONTACT_ADDRESS CA ON C.ID = CA.CONTACT_ID
					GROUP BY C.ID,C.CONTACT_CODE,C.CONTACT_CATEGORY_ID,C.CONTACT_GROUP,C.CONTACT_NAME,C.CONTACT_PERSON,
					C.CONTACT_NO,C.EMAIL_ADDRESS,C.OFFICE_ADDRESS,C.FACTORY_ADDRESS,C.BALANCE_AMOUNT,C.IS_CUSTOMER,C.CUSTOMER_USER_ID,
					C.CUSTOMER_DATE,C.IS_SELLER,C.SELLER_USER_ID,C.SELLER_DATE,C.IS_ACTIVE,C.CREATE_USER,C.CREATE_DATE,C.UPDATE_USER,C.UPDATE_DATE,
					C.REVISE_NO,C.RowVersion,EVT.VALUE_NAME
                    ORDER BY C.CONTACT_CATEGORY_ID,C.CONTACT_GROUP,C.CONTACT_NAME";

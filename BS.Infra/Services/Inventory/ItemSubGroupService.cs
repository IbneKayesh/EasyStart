namespace BS.Infra.Services.Inventory
{
    public class ItemSubGroupService
    {
        ...
        {
            FormattableString sql = $@"SELECT ISG.*,IG.ITEM_GROUP_NAME
FROM ITEM_SUB_GROUP ISG
JOIN ITEM_GROUP IG ON ISG.ITEM_GROUP_ID = IG.ID
ORDER BY ISG.CODE_PREFIX,IG.ITEM_GROUP_NAME,ISG.IS_FG_RM,ISG.IS_STOCK, ISG.LEAD_DAYS";
            return dbCtx.Database.SqlQuery<ITEM_SUB_GROUP_VM>(sql).ToList();
        }
